...
exports.handler = vandium.generic()
  .handler( (event, context, callback) => {
    ...
    var sql = "SELECT * FROM questions b WHERE id IS NOT NULL";
    if(search != ''){
       sql += " AND b.name LIKE '%" + search + "%'";
    }
    if(role != ''){
       sql += " AND id IN(SELECT question_id FROM questions_roles WHERE role_id IN(SELECT id FROM roles WHERE name = '" + role + "'))";
    }   
    if(tags != ''){
